...
セッション情報・リポジトリ一覧は全て `~/.claude/` から導出する。リポジトリの登録ステップは不要。

これにより:
- 初回起動時に過去の全セッションが即座に表示される（リポ登録不要）
- Claude Code のデータが single source of truth
- Yuru 側でセッションやリポジトリの状態を二重管理しない
...
リポジトリの登録ステップは不要。`~/.claude/projects/` をスキャンして、過去に Claude Code で作業した全リポジトリのセッションを自動検出する。

- 初回起動時: 過去の全セッションが即座に表示される
- 新規リポジトリ: [+] → フォルダ選択ダイアログでリポジトリを選択 → セッション開始と同時に `~/.claude/` に記録される
- `~/.yuru/` にリポジトリ一覧を持たない。`~/.claude/` が single source of truth
...
現時点では Yuru 独自の永続化データは持たない。必要が生じた場合は `~/.yuru/` に以下を導入する:

- `state.json`: ウィンドウサイズ・位置、最後に選択していたセッション、列幅等の UI 状態
- `config.json`: ユーザー設定（テーマ、キーバインド等）

セッションやリポジトリの情報は引き続き `~/.claude/` から導出し、二重管理しない。
