...
polling (`startPollingLoop`) は固定間隔で回し、ウィンドウのフォーカス・再表示では次の予約を
待たずに即座に 1 回実行する。git status /
...
この省エネ方針は `YURU_SAVE_ENERGY=0` を付けて起動すると無効になり、polling はフォーカスを
考慮しなくなる。画面の鮮度への影響を比較するためのスイッチであり、
...
diff パネルは Markdown / HTML / 画像 / 音声・動画をプレビューとして描画でき、それ以外は差分テキストとして見せる。
画像・音声・動画は中身を文字列にできないので、テキストの diff document (path と両側の文字列) には
載せず、別の IPC で両側の**大きさと URL**を取得する。3 種類とも同じ IPC・同じ枠 (ラベル・中身・
メタ情報の 3 段、両側あれば 2 カラム) を使い、違うのは中身をどの要素で描くかだけである。
取得の契機は差分テキストと同じで、変更ありのファイルと worktree 外のファイルは同じ間隔で取り直す。
`Reviewed` は押した時点の内容を記録するので、画面が古いまま取り残されると見ていない内容を承認できて
しまうためである。同じ理由で、ファイルや scope を切り替えた時は前の内容を残さない。
scope の解釈はテキストと同じ差分ローダを使うので、`Committed / Staged / Unstaged` と合算 diff の
どれでも同じように前後を出せる。

中身はアプリのメモリを通さない。画面自体が `file:` オリジンなので、作業ツリーのファイルは `file://`
でそのまま指せて、Chromium が必要なぶんだけディスクから読む。元側だけは作業ツリーに存在しないので、
blob OID をファイル名にして一時ディレクトリへ一度だけ書き出し、そのファイルを指す。OID は不変なので
中身が古くなることはなく、使い回せる。取り出したファイルは自分では消さない (`$TMPDIR` なので macOS が
古いものを掃除する)。Yuru を複数起動しても互いのファイルに触らない。
URL には中身の識別子 (git の blob なら OID、作業ツリーのファイルならサイズと mtime) をクエリとして
付ける。Chromium は URL 全体でキャッシュするので、パスが変わらない作業ツリーのファイルは、これがないと
書き換えられても古い内容を表示し続ける。逆に言えば URL が「どの中身か」を表すので、両側が同じ内容かの
判定も URL の比較で済む。

画像は両側の最大寸法を共通の座標系として同じ倍率で置くので、寸法が変わったことも見た目の差と一緒に
分かる。重ね合わせや画素差の表示は、フロントエンドの見た目を詰める場面以外では必要にならないため
作っていない。SVG は画像として描画できるテキストなので、プレビューでは描画、閲覧では差分テキストになる。

音声・動画は再生そのものを `<audio>` / `<video>` のコントロールに任せる。前後で時間軸を共有する
プレイヤーは作っていない。比べる手がかりは並んだメタ情報 (長さ、動画は寸法、サイズ) で、サイズは
git から分かる値、長さと寸法は要素に読み込ませて分かる値である。画像が `<img>` の自然サイズを測るのと
同じ位置にあたる。中身が差し替わった時は、再生位置と再生状態を引き継いでから読み直す。
poll で運ぶのはメタデータ (大きさと「中身が変わっていないこと」を表す識別子) だけで、識別子が
変わった時に読み直す。作業ツリー側は中身が変わっても URL が変わらないので、URL ではなく識別子を見て
読み直す。読み直しでは再生位置と再生状態を保つので、agent が作り直した音を同じ場所から聴き直せる。
識別子は git の blob なら OID、作業ツリーのファイルならサイズと mtime で、場所が違うもの同士では
比較しない (両側が同じ内容かどうかは git の状態から導く)。
形式は拡張子から Chromium が判断するので、対象の拡張子でも再生できないことがある。
その場合はその側だけ理由を出して再生 UI を出さない。
