...
- 共有 reader の初回読み取りとファイル置換/truncate 後の再初期化は、JSONL の末尾から遡って最新の assistant message だけを拾う (`readLatestJsonlEntry`)。巨大なセッションログで全件走査によるクラッシュや初回表示の遅延が起きたため。カーソルは末尾の完全な行の直後に置かれ、以後は追記分だけを読む。listener には tail 初期化の結果は通知しない
