...
test("yuru session transcript-path は現在の worktree を渡して path を表示する", async (t) => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "yuru-cli-session-transcript-"));
  const socketPath = path.join(tempDir, "api.sock");
  const requests = [];
  const server = await startApiServer({
    socketPath,
  ...
  const result = await runCli(["session", "transcript-path"], {
    ...process.env,
    YURU_API_SOCKET: socketPath,
  ...
  assert.deepEqual(result, {
    code: 0,
    stdout: "/sessions/parent.jsonl\n",
    stderr: "",
  });
  assert.deepEqual(requests, [
    {
      command: "session.transcript-path",
      args: { worktreePath: "/repo/.yuru/worktrees/current-task" },
    },
  ]);
});

test("yuru session transcript-path は明示した worktree を優先する", async (t) => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "yuru-cli-session-transcript-"));
  const socketPath = path.join(tempDir, "api.sock");
  const requests = [];
  const server = await startApiServer({
    ...
    handleRequest(request) {
      ...
      return {
        ok: true,
        data: { transcriptPath: "/sessions/explicit.jsonl" },
      };
    },
  });
  t.after(async () => {
  ...
  const result = await runCli(
    [
      ...
      "transcript-path",
      "--worktree",
      "/repo/.yuru/worktrees/explicit-task",
    ],
    {
      ...process.env,
      YURU_API_SOCKET: socketPath,
  ...
  assert.equal(result.code, 0);
  assert.deepEqual(requests[0].args, {
    worktreePath: "/repo/.yuru/worktrees/explicit-task",
  });
});

test("yuru session transcript-path は worktree 未指定を拒否する", async () => {
  const env = { ...process.env, YURU_API_SOCKET: "/unused/api.sock" };
  delete env.YURU_WORKTREE_PATH;

  const result = await runCli(["session", "transcript-path"], env);

  assert.deepEqual(result, {
    code: 1,
    stdout: "",
    stderr: "This command requires --worktree outside a Yuru task worktree terminal.\n",
  });
});
